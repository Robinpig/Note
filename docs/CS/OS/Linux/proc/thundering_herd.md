...
惊群效应(thundering herd) 指多进程(线程) 在同时阻塞等待同一个事件时候, 
如果等待事件发生 会唤醒所有的等待进程(线程) 但是最终只能有一个进程(线程)获得控制权处理这个事件 其它竞争者只能重新进入等待
惊群效应使得Linux内核对用户进程(线程)频繁进行无效调度、上下文切换 消耗系统资源 降低性能

Linux 2.6 版本之前 监听同一个socket的进程会挂在同一个等待队列上 请求到来后会唤醒所有等待的进程
2.6 版本之后 通过引入了一个标志位 WQ_FLAG_EXCLUSIVE 解决掉了 accept的惊群效应
