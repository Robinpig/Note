...
内核实现了⽹络层的1P协议，也实现了传输层的TCP协议和UDP协议。这些协议对应的实现西数分别是 1DrCv0、tcp_v4_rcv 和 udprev() 
fs_initcall调⽤inet init 后开始⽹络协议栈注册，通过inet init， 将这些函
数注册到inet Drotos和ptype base数据结构中
