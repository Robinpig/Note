...
Redis 提供三种持久化方式：RDB 快照、AOF 日志以及混合持久化
...
2. fork 出 bgsave 子进程时，fork会阻塞主线程，主线程的内存越大，阻塞时间越长。
...
- debug reload 命令重新加载 Redis 会触发 bgsave
- 默认情况下执行 shutdown 命令，如果没有开启AOF持久化，也会触发 bgsave操作


其它配置
```
# 持久化出错时 主进程停止写操作，例如操作系统权限，磁盘已满
stop-writes-on-bgsave-error yes

# 文件末尾 64位 CRC校验码，这个功能会有一定的性能降低，但能提高数据可靠性
rdbchecksum yes
```
...
* Redis [fork](/docs/CS/OS/Linux/proc/process.md?id=fork) 一个子进程，现在我们有了一个子进程和一个父进程。
* 子进程开始将新的Base AOF 写入临时文件。
* 父进程打开一个新的Inc AOF 文件以继续写入更新。如果重写失败，旧的基础文件和增量文件（如果有的话）加上这个新打开的增量文件代表完整的更新数据集，因此我们是安全的。
...
新的 Multi Part AOF 机制在重写时无需 AOF Rewrite 缓冲区，不需要重复写AOF文件
...
生产环境中 RDB 和 AOF 通常同时开启。数据恢复时 Redis **优先使用 AOF 文件**，因为其数据更完整；只有在 AOF 关闭时才会使用 RDB 恢复
