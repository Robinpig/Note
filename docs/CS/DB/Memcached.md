...
[Memcached](https://www.memcached.org/) 是一个内存键值存储系统，用于存储数据库调用、API 调用或页面渲染结果中的任意小数据块（字符串、对象）。
它是一个免费开源、高性能、分布式的内存对象缓存系统，本质上是通用的，但旨在通过减轻数据库负载来加速动态 Web 应用程序。
