...
Dubbo 的架构采用 分层设计，共 10 层，从上到下：

| 层级 |	职责  |
| --- | --- |
| Service	| 业务接口层，定义服务契约  |
| Config	| 配置层，以 ServiceConfig/ReferenceConfig 为中心  |
| Proxy	| 代理层，透明化远程调用（生成客户端 Stub / 服务端 Skeleton）  |
| Registry |	注册层，封装服务地址的注册与发现（ZooKeeper、Nacos 等）  |
| Cluster |	集群层，封装路由、负载均衡、容错，将多个 Provider 聚合成一个 Invoker  |
| Monitor |	监控层，统计调用次数和耗时  |
| Protocol |	RPC 调用层，Invoker 是核心模型，代表可执行体  |
| Exchange |	信息交换层，封装请求-响应模式，同步转异步  |
| Transport	| 网络传输层，抽象 Netty / Mina 为统一接口  |
| Serialize	| 序列化层，可插拔（Hessian、JSON、Protobuf 等）  |

核心调用流程：

1. Provider 启动 → 通过 Registry 注册服务地址
2. Consumer 启动 → 从 Registry 订阅服务列表，与 Provider 建立长连接
3. Consumer 调用 → Proxy 通过负载均衡选一台 Provider，经 Protocol → Exchange → Transport 发起 RPC
4. Provider 处理 → Transport 反序列化请求，层层向上到达 Service 实现类
5. 结果返回 → 沿原链路返回，Monitor 记录统计数据
6. 地址变更 → Registry 实时推送变更给 Consumer

设计亮点：
- 微内核+插件：所有层都通过 SPI 机制可扩展，协议、序列化、注册中心均可自由替换
- Invoker 核心抽象：所有模型都向 Invoker 靠拢——本地/远程/集群调用统一抽象
- 面向 URL 设计：所有信息（地址、配置、元数据）都编码在 URL 中，贯穿全链路
