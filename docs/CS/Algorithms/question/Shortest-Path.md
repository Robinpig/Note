...
最短路径的4个常用算法是 Floyd、Bellman-Ford、SPFA、Dijkstra
...
时间复杂度上限为 $O(n^2)$(朴素),在实际应用中较为稳定;加上堆优化之后更是具有 $O((n+m)logn)$的时间复杂度,在稠密图中有不俗的表现
