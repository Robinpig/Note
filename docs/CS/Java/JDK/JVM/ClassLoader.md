...
当你编译一个 .java 源文件时，它会被转换为 .class 文件的字节码。

JVM类加载的时机主要基于 **首次主动使用** 的原则
每次创建 Java 类的实例时，该类必须首先被加载到内存中。
当你在程序中尝试使用某个类时，类加载器会将其加载到主内存中。
类加载器通常会搜索一些核心 Java 库以及 CLASSPATH 环境变量中包含的所有目录。
如果找不到所需的类，它会抛出 `java.lang.ClassNotFoundException`。
第一个被加载到内存中的类通常是包含 `main()` 方法的类。

它主要负责三项活动。

- **Loading** 是查找具有特定名称的类或接口类型的二进制表示，并从该二进制表示创建类或接口的过程。
- **Linking** 是将类或接口合并到 Java 虚拟机的运行时状态以便其执行的过程。
  1. **Verify** – 字节码验证器将验证生成的字节码是否正确，如果验证失败，将收到验证错误。
  2. **Prepare** – 为所有静态变量分配内存并赋予默认值。
  3. **Resolve** – 将所有符号内存引用替换为来自 Method Area 的原始引用。
- **Initialization** 包括执行类或接口的初始化方法 `<clinit>`。
  这是类加载的最后阶段；在此阶段，所有静态变量将被赋予原始值，并执行静态块。
...
**每个 ClassLoader 实例都有一个关联的 parent class loader。**
当请求查找类或资源时，ClassLoader 实例会**在尝试自行查找类或资源之前**，将类或资源的搜索委托给其 parent class loader。

Java 运行时具有以下内置类加载器：

- Bootstrap class loader。它是虚拟机的内置类加载器，通常表示为 null，没有 parent。
- Platform class loader。所有平台类对 platform class loader 可见，它可以用作 ClassLoader 实例的 parent。
  平台类包括 Java SE 平台 API、其实现类以及由 platform class loader 或其祖先定义的 JDK 特定运行时类。<br/>
  为了允许升级/覆盖在 platform class loader 中定义的模块，并且当升级后的模块读取由 platform class loader 及其祖先以外的类加载器定义的模块时，
  platform class loader 可能需要委托给其他类加载器，例如 application class loader。
  换句话说，由 platform class loader 及其祖先以外的类加载器定义的命名模块中的类可能对 platform class loader 可见。
- System class loader。也称为 application class loader，与 platform class loader 不同。
  system class loader 通常用于定义应用程序类路径、模块路径和 JDK 特定工具上的类。
  platform class loader 是 system class loader 的 parent 或祖先，所有平台类对其可见。
...
支持并发加载类的类加载器称为 parallel capable class loaders，它们需要在类初始化时通过调用 ClassLoader.registerAsParallelCapable 方法注册自身。
注意，ClassLoader 类默认注册为 parallel capable。
但是，其子类如果支持并行加载，仍然需要注册自身。
在委托模型不严格分层的环境中，类加载器需要支持并行加载，否则类加载可能导致死锁，因为在类加载过程中会持有加载器锁（参见 loadClass 方法）。

在委托模型不严格分层的环境中，类加载器需要支持并行加载，否则类加载可能导致死锁，因为在类加载过程中会持有加载器锁（参见 `loadClass` 方法）。

使用指定的二进制名称加载类。此方法的默认实现按以下顺序搜索类：

1. 调用 findLoadedClass(String) 检查类是否已加载。
2. **调用 parent class loader 上的 loadClass 方法。** 如果 parent 为 null，则使用虚拟机内置的类加载器。
3. 调用 findClass(String) 方法查找类。

如果使用上述步骤找到了类，并且 resolve 标志为 true，则此方法将在生成的 Class 对象上调用 `resolveClass(Class)` 方法。
鼓励 ClassLoader 的子类重写 `findClass(String)`，而不是此方法。
除非被重写，否则此方法会在整个类加载过程中对 getClassLoadingLock 方法的结果进行同步。
...
1. 如果此 ClassLoader 对象注册为 parallel capable，则该方法返回**与指定类名关联的专用对象**。
2. 否则，该方法返回**此 ClassLoader 对象**。
...
通常，Java 虚拟机以平台相关的方式从本地文件系统加载类。
例如，在 UNIX 系统上，虚拟机从 CLASSPATH 环境变量定义的目录加载类。
然而，某些类可能并非来自文件；它们可能来自其他来源，例如网络，或由应用程序构造。
defineClass 方法将字节数组转换为 Class 类的实例。
这个新定义的类的实例可以使用 Class.newInstance 创建。
由类加载器创建的对象的方和构造器可能引用其他类。
为了确定被引用的类，Java 虚拟机调用最初创建该类的类加载器的 loadClass 方法。

例如，应用程序可以创建一个网络类加载器来从服务器下载 class 文件。示例代码如下：
...
网络类加载器子类必须定义 findClass 和 loadClassData 方法来从网络加载类。
一旦下载了构成类的字节，它应该使用 defineClass 方法来创建类实例。
示例实现如下：
...
使用 getSystemClassLoader() 方法返回的 ClassLoader 作为 parent class loader 创建新的类加载器。
如果存在安全管理器，将调用其 checkCreateClassLoader 方法。这可能导致安全异常。
...
调用 `java.lang.ClassLoader.getSystemClassLoader()` 并初始化 AppClassLoader 及其 parent
...
- Loading 在链接之前完全完成
  ...
  - 创建 InstanceKlass
    - 初始化 itable 偏移表
    - fill_oop_maps
    - 创建 mirror（java.lang.Class 实例）
- Linking
  - Verification
  - 验证之后、类的第一个方法执行之前的 Rewriting
  - 类重写后链接方法入口点（解释器和编译器）
  - 方法重写后初始化 vtable 和 itable
  - Preparation 可在创建后的任何时间发生，但必须在初始化之前完成
  - Resolution
- Initialization
  - 调用 `clinit` 方法（如果存在）
- Using
...
Loading 涉及获取具有特定名称的类或接口的二进制表示（字节码），并从中生成原始的类或接口。

Java 有三种内置类加载器：

1. BootStrap ClassLoader – 这是根类加载器。它是 Extension Class Loader 的超类，加载标准 Java 包，如 java.lang、java.net、java.util、java.io 等。
   这些包存在于 rt.jar 文件和 $JAVA_HOME/jre/lib 目录中的其他核心库中。
2. Extension ClassLoader – 这是 Bootstrap Class Loader 的子类，也是 Application Class Loader 的超类。它加载位于 $JAVA_HOME/jre/lib/ext 目录中的标准 Java 库扩展。
3. Application ClassLoader – 这是最终的类加载器，是 Extension Class Loader 的子类。它加载 classpath 上的文件。
   默认情况下，classpath 被设置为应用程序的当前目录。classpath 也可以通过添加 -classpath 或 -cp 命令行选项来修改。

上述 ClassLoaders 在加载 class 文件时将遵循委托层次结构算法。

JVM 使用 ClassLoader.loadClass() 方法将类加载到内存中。它尝试基于完全限定名称加载类。
此方法的默认实现按以下顺序搜索类：

1. 调用 findLoadedClass(String) 检查类是否已加载。
2. 在 parent class loader 上调用 loadClass 方法。如果 parent 为 null，则使用虚拟机内置的类加载器。
3. 调用 findClass(String) 方法查找类。

如果 parent class loader 无法找到类，它将工作委托给子 class loader。如果最后一个子 class loader 也无法加载该类，则抛出 NoClassDefFoundError 或 ClassNotFoundException。
...
*链接指定的类。这个（命名容易引起误解的）方法可以被 class loader 用来链接类。如果类 c 已经链接，则此方法直接返回。否则，类将按如下所述进行链接。
...
defineClass1 和 defineClass2 都调用 JVM_DefineClassWithSource -> jvm_define_class_common
...
分配 [Constant Pool](/docs/CS/Java/JDK/JVM/Oop-Klass.md?id=Constant-Pool)
