...
它使用这些信息来确定暂停中完成的工作量。例如，G1 首先在最高效的区域回收空间（即主要充满垃圾的区域，因此得名）。

G1 主要通过 evacuation 回收空间：在选定要回收的内存区域中发现的存活对象被复制到新的内存区域，在此过程中压缩它们。
在 evacuation 完成后，先前由存活对象占用的空间被重新用于应用程序分配。

Garbage-First 收集器不是实时收集器。
它尝试在较长时间内以高概率满足设定的暂停时间目标，但并不总是对给定的暂停具有绝对确定性。
...
G1 将堆划分为一组大小相等的堆区域（heap region），每个区域是虚拟内存的一个连续范围，如图 1 所示。
区域是内存分配和内存回收的单位。
在任何给定时间，每个区域可以是空的（浅灰色），或分配给特定代（年轻代或老年代）。当内存请求到来时，内存管理器分配空闲区域。
内存管理器将它们分配给一个代，然后作为可用空间返回给应用程序，以便应用程序可以自行分配。
...
我们对堆区域类型的值进行编码，以便可以快速确定代。
标签分为两部分：

- 主要类型（young、old、humongous）：高 N-1 位
- 次要类型（eden/survivor、starts/cont hum 等）：低 1 位

如果将来需要增加次要类型的数量，我们将不得不增加后者的大小，从而减小前者的大小。
...
年轻代包含 eden 区域（红色）和 survivor 区域（带"S"的红色）。
这些区域提供与其他收集器中各自连续空间相同的功能，不同之处在于 G1 中这些区域通常在内存中以非连续模式布局。
老年代区域（浅蓝色）组成老年代。老年代区域可能包含大对象（带"H"的浅蓝色），用于跨越多个区域的对象。

堆区域中的分配包括递增已分配和未分配空间之间的边界 top。
一个区域是当前分配区域，从中分配存储。
由于我们主要关注多处理器，mutator 线程仅使用比较并交换（CAS）操作在此堆区域中直接分配线程本地分配缓冲区（TLAB）。
然后它们在这些缓冲区内私有地分配对象，以最小化分配争用。
当当前分配区域被填满时，选择一个新的分配区域。空区域被组织成链表，使区域分配成为常数时间操作。
较大的对象可能直接在当前分配区域中分配，在 TLAB 之外。
然而，大小超过堆区域大小 3/4 的对象被称为大对象（humongous）。
...
每个区域都有一个关联的 remembered set，指示所有可能包含指向区域内（存活）对象的指针的位置。
维护这些 remembered set 要求 mutator 线程在它们进行可能创建跨区域指针的指针修改时通知收集器。
此通知使用卡表：堆中的每 512 字节 card 映射到卡表中的一个字节条目。
每个线程都有一个关联的 remembered set 日志，即当前缓冲区或已修改 card 的序列。
此外，还有一个全局的已填充 RS 缓冲区集合。

Remembered set 本身是 card 的集合（由哈希表表示）。
实际上，由于并行性，每个区域都有一个由多个此类哈希表组成的关联数组，每个并行 GC 线程一个，以允许这些线程无干扰地更新 remembered set。
Remembered set 的逻辑内容是每个组件哈希表表示的集合的并集。
Remembered set 写屏障在指针写入后执行。
如果代码执行指针写入 x.f = y，并且寄存器 rX 和 rY 分别包含对象指针值 x 和 y，则屏障的伪代码如下：
...
此屏障使用了 Stefanovi´c 等人简要提及的过滤技术。
如果写入在同一个堆区域中创建从对象到另一个对象的指针（我们预计这是常见情况），则无需在 remembered set 中记录。
第 1 和 2 行的异或和移位意味着如果 x 和 y 在同一个堆区域中，则 rTmp 在第二行后为零。
第 4 行添加了对空指针存储的过滤。
如果存储通过了这些过滤检查，则它创建了一个跨区域指针。
rs 入队例程读取对象头部 rX 的卡表条目。
如果该条目已经 dirty，则不执行任何操作。
这减少了多次存储到同一 card 的工作量，这是由于初始化写入造成的常见情况。
如果卡表条目不 dirty，则将其标记为 dirty，并将指向该 card 的指针入队到线程’s remembered set 日志中。
如果此次入队填满了线程的当前日志缓冲区（默认容纳 256 个元素），则将该缓冲区放入全局已填充缓冲区集合，并分配一个新的空缓冲区。’s current log buffer (which holds 256 elements by default), then that buffer is put in the global set of filled buffers, and a new empty buffer is allocated.

并发 remembered set 线程等待（通过条件变量）已填充 RS 缓冲区集的大小达到可配置的启动阈值（默认为 5 个缓冲区）。
Remembered set 线程将已填充缓冲区作为队列处理，直到队列长度减少到启动阈值的 1/4。
对于每个缓冲区，它处理每个卡表指针条目。
某些 card 是热的：它们包含频繁写入的位置。
为避免重复处理热 card，我们尝试识别最热的 card，并将其处理推迟到下一次 evacuation 暂停。
我们通过第二个卡表来实现这一点，该卡表记录自上次 evacuation 暂停以来该 card 被弄脏的次数（在此期间，此表与卡表本身一样被清除）。
当我们处理 card 时，我们在此表中增加其计数。
如果计数超过热度阈值（默认 4），则将该 card 添加到称为热队列的循环缓冲区（默认大小为 1K）。
此队列在每个 evacuation 暂停开始时像日志缓冲区一样被处理，因此在结束时为空。
如果循环缓冲区已满，则从另一端逐出 card 并处理。
...
G1BarrierSet: public CardTableBarrierSe
...
## Young collect
Eden 区满 G1会根据停顿预测模型计算当前Eden区GC大概耗时 
- 如果回收时间远小于 -XX:MaxGCPauseMills 则分配空闲分区加入Eden区
- 如果时间接近 则触发一次Young GC
...
G1ParScanThreadState::copy_to_survivor_space(InCSe
...
初始标记是以 Young GC 后的 survivor 作为 root 进行扫描 处理的是 survivor 分区到 老年代 region的引用
...
G1IHOPControl* G1Policy::create_ihop_control(const G1Predic
...
G1CollectedHeap::do_concurrent_mark() {
...
ool G1Policy::next_gc_should_be_mixed(const char*
...
当晋升失败、疏散失败、大对象分配失败、Evac失败时，可能触发Full GC，在JDK10之前，Full GC是串行的，JEP 307: Parallel Full GC for G1之后引入了并行Full GC

bool G1CollectedHeap::do_full_collection(bool
...
- 标记对象，具体逻辑在[G1FullGCMarkTask](/docs/CS/Java/JDK/JVM/GC/G1.md.md?id=G1FullGCMarkTask)
- 清理弱引用
