...
MCP遵循客户端-服务器架构，其中 MCP Host——[Claude Code](https://www.anthropic.com/claude-code) 或 Claude Desktop 等AI应用程序——与一个或多个MCP Server 建立连接。MCP 主机通过为每个 MCP Server 创建一个 MCP Client 来实现这一目标。每个 MCP Client 都与相应的 MCP Server 保持专用的一对一连接。MCP架构的主要组成者是：
