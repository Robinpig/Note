...
int main()
{
    ...
    for (system("title 得分:0"); 1; Sleep(100)) ///初始化计分板，延迟
    {
        ...
        if (map[hY * Width + hX] > 1 || map[hY * Width + hX] == -2)
            break;                      //判断是否吃到自己
...
/*67行贪吃蛇，在原有的47行贪吃蛇上完善和拓展功能：
 1.自定义地图大小	(+2 lines)(4,14)宏定义实现
 2.地图元素--墙		(+1 line)(58)在原有的基础上增加规则，实现也不难
 3.墙可自定义分布	(+2 lines)(11-12)也许不止这两行
 4.可生成多个食物	(+6 lines)(16-21)反复生成
 5.重新开始			(+5 lines)(8-9,64-66)直接套一个循环，结尾加个标题提示
 6.暂停和退出		(+2 lines)(31-32)增加判断就行
 7.计分板			(+2 lines)(48-49)在标题栏上实现*/
